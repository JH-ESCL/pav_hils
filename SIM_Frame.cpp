                             ...
                             bool use_drag)
{
    ...
    /* //Torque Thrust 1st order_JH20240528
    ...
    for (uint8_t i=0; i<num_motors; i++) {
       ...
       /* // Apply first-order filter to torque and thrust_JH_240528
        for (uint8_t j = 0; j < 3; j++) {
            mtorque_real[j] += alpha * (mtorque[j] - mtorque_real[j]);
            mthrust_real[j] += alpha * (mthrust[j] - mthrust_real[j]);
        }
        ...
        AP::logger().Write_SITL(thrust[2],torque[2]);
    ...
    // Check if 333ms (3Hz) has passed since the last message
    if (current_time - last_send_time_ms >= 3000) {
        last_send_time_ms = current_time; // Update the last send time
            char buf[50];
        snprintf(buf, sizeof(buf), "p': %.3f", rot_accel.x);
        gcs().send_text(MAV_SEVERITY_INFO, "%s", buf);
                char buf2[50];
        snprintf(buf2, sizeof(buf2), "q': %.3f", rot_accel.y);
        gcs().send_text(MAV_SEVERITY_INFO, "%s", buf2);
                char buf3[50];
        snprintf(buf3, sizeof(buf3), "r': %.3f", rot_accel.z);
        gcs().send_text(MAV_SEVERITY_INFO, "%s", buf3);
                char buf4[50];
        snprintf(buf4, sizeof(buf4), "tot_thr : %.3f", thrust[2]);
        gcs().send_text(MAV_SEVERITY_INFO, "%s", buf4);
                char buf5[50];
        snprintf(buf5, sizeof(buf5), "torq_z: %.3f", torque[2]);
        gcs().send_text(MAV_SEVERITY_INFO, "%s", buf5);
    }
