...
// Sensor_Noise_JH_20240527
   // Apply the obtained noise values for gyro and accelerometer
    const float gyro_noise_x = 0.002; // Gyroscope noise for X axis in rad/s
    const float gyro_noise_y = 0.002; // Gyroscope noise for Y axis in rad/s
    const float gyro_noise_z = 0.002; // Gyroscope noise for Z axis in rad/s
    
    const float accel_noise_x = 0.3; // Accelerometer noise for X axis in m/s²
    const float accel_noise_y = 0.3; // Accelerometer noise for Y axis in m/s²
    const float accel_noise_z = 0.3; // Accelerometer noise for Z axis in m/s²

    gyro.x += rand_normal(0, 1) * gyro_noise_x ;
    gyro.y += rand_normal(0, 1) * gyro_noise_y ;
    gyro.z += rand_normal(0, 1) * gyro_noise_z ;

    accel_body.x += rand_normal(0, 1) * accel_noise_x ;
    accel_body.y += rand_normal(0, 1) * accel_noise_y ;
    accel_body.z += rand_normal(0, 1) * accel_noise_z ;
