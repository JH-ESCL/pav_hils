...
float last_pwm = 1000;
float pwm_ = 1000;
//float motor_time_const = 0.161;
float motor_time_const = 0.01;

float tau_AT = 0.01;
//float K_AT = 0.0727 ; //0.7425 kgf cm -> 0.0727 Nm
float K_AT = 0.5 ; //0.7425 kgf cm -> 0.0727 Nm

float Anti_Torque = 0; // This is the state variable representing the torque

// 2nd order State-space representation variables
//float x1_AT = 0; // state variable x1
//float x2_AT = 0; // state variable x2
                             ...
                             bool use_drag)
{
    ...
    if (last_calc_us != 0 && slew_max > 0) {
        ...
        float alpha = dt / (motor_time_const + dt);
        pwm_ = pwm + alpha * (pwm - last_pwm);
    ...
    //float motor_thrust = calc_thrust(command, air_density, velocity_in, voltage_scale);
        //Thrust from pwm_JH20240525
    //float motor_thrust = (0.0007 * pwm_ * pwm_ - 1.096 * pwm_ + 368 + 28) + (0.0007 * pwm_ * pwm_ - 1.096 * pwm_ + 368 + 28) * 0.8;  // motor*2
    //float motor_thrust = (0.0007 * pwm_ * pwm_ - 1.096 * pwm_ + 368 + 28);  // motor
    float motor_thrust = 25*((pwm_-1000)/1000);  // motor AROT 4114-11 KV 320 and Dualsky Prop 15 5.5 MR Prop for XM5010/5015
    //float motor_thrust = 25.4*pwm_;  // motor AROT 4114-11 KV 320 and Dualsky Prop 15 5.5 MR Prop for XM5010/5015
    ...
    // Yaw torque from PWM _ JH20240628
    //float dt = (now_us - last_calc_us)*1.0e-6;
    //first order
    //float dAT = (K_AT * pwm - Anti_Torque) / tau_AT;
    //float dAT = (K_AT * (pwm-1000)/1000 - Anti_Torque) / tau_AT; //Counter torque Octo

    //Anti_Torque = Anti_Torque + dAT * dt;
    Anti_Torque = 0.5*(pwm_-1000)/1000;
    //Vector3f rotor_torque = thrust_vector * yaw_factor * Anti_Torque * -2.0; // two motors in each frame 
    Vector3f rotor_torque = thrust_vector * yaw_factor * Anti_Torque * -1.0; // two motors in each frame 
